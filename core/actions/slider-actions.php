...
function response_slider_lite_content() {
	...
	global $ne_themename, $ne_themeslug, $options, $wp_query, $post, $slider_default, $ne_root;
	...
	if (is_page()) {
		...
		$slide1source = $options->get($ne_themeslug.'_blog_slide_one_image');
		$slide2source = $options->get($ne_themeslug.'_blog_slide_two_image');
		$slide3source = $options->get($ne_themeslug.'_blog_slide_three_image');
		
		$slide1 = $slide1source['url'];
		$slide2 = $slide2source['url'];
		$slide3 = $slide3source['url'];
	
		$link1 = $options->get($ne_themeslug.'_blog_slide_one_url');
		$link2 = $options->get($ne_themeslug.'_blog_slide_two_url');
		$link3 = $options->get($ne_themeslug.'_blog_slide_three_url');
